...
void power::measureTask()
{
    ...
    bool isSequenceStartMistake = control::sequenceStart.isManualRaised() && !control::dump.isManualRaised() && !sequence::emergencyStopSequenceIsActive;
...
void control::handleManualTask()
{
    ...
    if (control::sequenceStart.isManualRaised() && !control::dump.isManualRaised() && !sequence::emergencyStopSequenceIsActive)
    {
        mp3_play(13);
        return;
    }
...
void control::setEmergencyStop()
{
    ...
    control::dump.setAutomaticOff(); // シークエンス開始前にDUMPをCLOSEするため
